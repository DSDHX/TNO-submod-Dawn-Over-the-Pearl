...
- 简介区使用265×150可见滚动窗口和10000px文本上限；最长简介为白鹅新区晴空塔239字符，可完整滚动阅读。
- 简介和效果没有两个并排切换按钮。点击简介后显示同位置效果覆盖页，`effect_tooltip`预览的正是完工事件调用的实际奖励回调；点击覆盖页返回简介。
- `DOP_construction_show_project`、`DOP_construction_start_project`和`DOP_construction_show_and_start_project`只要求调用者设置临时项目ID；三个接口均以立法会式`effect_tooltip`显示动态项目名，并在`hidden_effect`中执行真实状态变更。
- 总署初始化只显示已有的经济建设菜单解锁提示；项目注册、迁移、动态修正和目录重建全部隐藏执行。三个建设动态修正均已补充中文名称与说明。
- 三微米冷战的每日激活必须先存在其自身`DOP_SCW_enabled`标志；解锁岭南建设总署不会附带启动三微米冷战。
- 重建国策“走出残垣断壁”不再直接赠送全州基础设施；完成后触发事件152、使用既有tooltip解锁岭南建设总署，并令ID 13广西高速公路网立项及立即开工。
